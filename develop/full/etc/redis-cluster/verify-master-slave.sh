...
# 获取所有主节点
masters=$(redis-cli -c -p 14161 cluster nodes | grep "master" | grep -v "fail" | awk '{print $1, $2}')

while read -r master_id master_addr; do
    echo "主节点: $master_addr ($master_id)"

    # 查找该主节点的从节点
    slaves=$(redis-cli -c -p 14161 cluster nodes | grep "slave" | grep "$master_id")
...
echo "测试主从同步..."
# 在主节点写入数据
redis-cli -c -p 14161 set replication_test "master_value"
